...
def graph_creator(db: list[str]) :
	...
	# with weight log(|documents in db containing u|)-log(|documents in db containing u and v|)
...
def dijkstra(graph: Graph, vertex, base_k) :
	...
	heapq.heappush(priority_queue, (0, vertex, base_k))

	while priority_queue :
		dist, current_vertex, k = heapq.heappop(priority_queue)
		# if distance[current_vertex] > dist :  # This is real way to Dijkstra but this make computer dead.
		# We're fine with a small approximation since what we have here is in fact a mathematical distance.
		if distance[current_vertex] > dist and k > 0 :
			distance[current_vertex] = dist
			for neighbour in graph.get_vertex(current_vertex).neighbours :
				next_vertex = neighbour.key
				heapq.heappush(priority_queue, (dist + graph.get_edge(current_vertex, next_vertex), next_vertex, k - 1))
